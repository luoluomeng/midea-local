...
class MideaACDevice(MideaDevice):
    ...
    def process_message(self, msg: bytes) -> dict[str, Any]:  # noqa: C901
        ...
        if (
            body_type in [ListTypes.B1, ListTypes.B5]
            and hasattr(message, str(DeviceAttributes.target_temperature))
            and hasattr(message, str(DeviceAttributes.indoor_temperature))
        ):
        ...
        for attr in self._attributes:
            if hasattr(message, str(attr)):
                if is_stale_c0_temperature and attr in [
